...
use engine_mlx_attention::gqa::{GqaConfig, GqaWeights};
use engine_mlx_attention::sliding_window::{SlidingWindowConfig, sliding_window_attention};
...
fn sliding_window_attention_bails_with_mlx_marker() {
    ...
    let cfg = SlidingWindowConfig { gqa, window_size: 512 };
    ...
    let err = sliding_window_attention(&null_ctx(), dummy_array(), &weights, &mut cache, &cfg, 0)
        .unwrap_err();
